  ...
  return (
    <section className="section text-black">
      <div className="container">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 text-center text-primary-700">
          {stats.map((stat) => (
            <div key={stat.key} className="group">
              <div className="text-3xl md:text-4xl font-bold text-black mb-2 group-hover:scale-110 transition-transform duration-300">
                {stat.value}
              </div>
              <p className="text-sm text-primary-700 md:text-base text-primary-100 font-medium">
                {t(stat.key)}
              </p>
            </div>
          ))}
